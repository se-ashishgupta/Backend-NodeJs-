...
const isAuthenticated = async (req, res, next) => {
  ...
  if (token) {
    const decoded = jwt.verify(token, "sdfggdfgdfgdfgdf");
...
app.post("/login", async (req, res) => {
  ...
  if (!isMatch)
    return res.render("login", { email, message: "Incorrect Passwrd" });

  const token = jwt.sign({ _id: user._id }, "sdfggdfgdfgdfgdf");
